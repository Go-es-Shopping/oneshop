...
| **賣家訂單 (拉出獨立，新增sellerorder.js)** | POST |`/api/seller/orders/` | `Order`, `Orderdetail`, `Product` | 建立訂單 (支援 Mock) |
| | GET | `/api/seller/orders/all` <br> GET `/api/seller/orders/` | `Order` | 查詢訂單列表 (支援 `OrderStatus` 篩選與 Mock) |
| | GET | `/api/seller/orders/:OrderID/details` | `Order`, `Orderdetail`, `Shipment`, `Payment`  | 查詢特定訂單的明細列表 |
...
| **賣家成效分析總覽** | GET | `/api/analytics/overview` | `Order`, `Orderdetail`, `Product`, `PageContent`, `PageVisit` | 取得賣家指定區間（預設 30 天）的營收、訂單數、動銷率、AOV、熱銷商品、滯銷商品與經營建議列表。 |
